...
from fed_utils import slave_run_train, evaluate
...
def main():
    ...
    if True:
        ...
        (pic["train_adj"], full_adj, pic["train_feats"], pic["test_feats"], pic["y_train"], y_val, y_test,
         ...
         visible_data) = utils.load_data(args.data_prefix, args.dataset, args.precalc, amazon=True)
    ...
    model = StackedGCN(args, pic["test_feats"].shape[1], pic["y_train"].shape[1],
    ...
    try:
        for epoch in range(args.epochs):
            ...
            print2file('Epoch: ' + str(epoch) + ' Average Train acc: ' + str(sum(epoch_acc)/len(epoch_acc)), args.logDir, True)

            if epoch % args.val_freq == 0:
                val_cost, val_acc, val_micro, val_macro = evaluate(model, args,
                ...
                if val_acc > best_val_acc:
                    ...
                    print2file("Best val_acc: " + str(best_val_acc) + " with epoch: " + str(epoch), args.logDir, True)

        torch.save(model.state_dict(), "./model/" + args.dataset + "-" + args.logDir[6:-4] + "Done.pt")
        ...
        # 1)
        test_cost, test_acc, micro, macro = evaluate(model, args, pic["test_features_batches"],
        ...
        # 2)
        test_model = StackedGCN(args, pic["test_feats"].shape[1], pic["y_train"].shape[1],
                           precalc=args.precalc,
                           num_layers=args.num_layers,
                           norm=args.layernorm)
        test_model.load_state_dict(torch.load(model_saved))
        test_model.eval()
        test_cost, test_acc, micro, macro = evaluate(test_model, args, pic["test_features_batches"],
                                                     pic["test_support_batches"], pic["y_test_batches"],
