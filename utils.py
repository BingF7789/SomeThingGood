...
def load_graphsage_data(dataset_path, dataset_str, normalize=True):
    ...
    graph_json = json.load(gfile.GFile('{}/{}/{}-G.json'.format(dataset_path, dataset_str, dataset_str)))
    graph_nx = json_graph.node_link_graph(graph_json)

    id_map = json.load(gfile.GFile('{}/{}/{}-id_map.json'.format(dataset_path, dataset_str, dataset_str)))
    is_digit = list(id_map.keys())[0].isdigit()
    id_map = {(int(k) if is_digit else k): int(v) for k, v in id_map.items()}

    class_map = json.load(gfile.GFile('{}/{}/{}-class_map.json'.format(dataset_path, dataset_str, dataset_str)))
    ...
    print('Removed %d nodes that lacked proper annotations due to networkx versioning issues', broken_count)

    feats = np.load(gfile.GFile('{}/{}/{}-feats.npy'.format(dataset_path, dataset_str, dataset_str), 'rb')).astype(
        np.float32)

    print('Loaded data (%f seconds).. now preprocessing..', time.time() - start_time)
    start_time = time.time()
    ...
    val_data = np.array([id_map[n] for n in graph_nx.nodes() if graph_nx.node[n]['val']], dtype=np.int32)
    test_data = np.array([id_map[n] for n in graph_nx.nodes() if graph_nx.node[n]['test']], dtype=np.int32)
    ...
    if normalize:
        train_ids = np.array(
            [id_map[n] for n in graph_nx.nodes() if not graph_nx.node[n]['val'] and not graph_nx.node[n]['test']])
    ...
    print('Data loaded, %f seconds.', time.time() - start_time)
    return num_data, train_adj, full_adj, feats, train_feats, test_feats, labels, train_data, val_data, test_data


def load_amazon_data(dataset_path, dataset):

    print("Initializing...")
    start_time = time.time()
    base_dir = dataset_path + "/" + dataset + "/"
    adj_file = base_dir + "adj_list.txt"
    val_idx_file = base_dir + "test_idx.txt"
    ...
    print("Prepare garph infomation")
    with open(adj_file, "r") as edge_f1:
        edge_info = edge_f1.readlines()

    for edge in edge_info:
        edge = edge.split()
        for node in edge[1:]:
            G.add_edge(int(edge[0]), int(node))
            edges.append((int(edge[0]), int(node)))

    num_data = G.number_of_nodes()

    print("Spliting Data..")
    with open(val_idx_file, "r") as v_idx_f1:
        ids = v_idx_f1.readlines()

    for id in ids:
        val_indx.append(int(id))

    val_data = np.array(val_indx)

    # get feature info
    feats = np.ones((num_data, 10))

    # get label info
    labels = []
    with open(label_file, "r") as label_f1:
        label_info = label_f1.readlines()

    for node_label in label_info:
        labels.append(node_label.split())
    ...
    print('Data loaded, %f seconds.', time.time() - start_time)
    return num_data, train_adj, full_adj, feats, train_feats, test_feats, np.array(
...
def load_data(data_prefix, dataset_str, precalc, amazon=False):
    """Return the required data formats for GCN models."""
    if not amazon:
        (num_data, train_adj, full_adj, feats, train_feats, test_feats, labels,
         train_data, val_data, test_data) = load_graphsage_data(data_prefix, dataset_str)
    else:
        (num_data, train_adj, full_adj, feats, train_feats, test_feats, labels,
         train_data, val_data, test_data) = load_amazon_data(data_prefix, dataset_str)

    visible_data = train_data   # fixme: why not just use train_data
...
def preprocess_multicluster_v2(adj,
                            parts,
                            features,
                            y_train,
                            train_mask,
    ...
    total_nnz = 0
    np.random.shuffle(parts)
    for _, st in enumerate(range(0, num_clusters)):
        pt = parts[st]
        for pt_idx in range(st + 1, min(st + block_size, num_clusters)):
            pt = np.concatenate((pt, parts[pt_idx]), axis=0)
            # print(type(pt))
...
def constract_train_data(p1, p2, data):
...
def masked_sigmoid_cross_entropy(preds, labels, criterion):
    """Sigmoid cross-entropy loss with masking."""
    pos_weight = torch.ones([preds.shape[1]])
...
def sigmoid_cross_entropy_with_logits(x, z):
    # implementation of tf.nn.sigmoid_cross_entropy_with_logists
    rs = max(x, 0) - x * z + math.log(1 + math.exp(-abs(x)))
    return rs
...
def masked_accuracy_multilabel(preds, labels, mask):
    ...
    preds = preds > 0
    labels = labels > 0.5
    incorrect_prediction = torch.ne(preds, labels)
    incorrect_all = incorrect_prediction.to(dtype=torch.float32)

    return 1 - torch.mean(incorrect_all)
...
def calc_f1_out(y_pred, y_true, multilabel):
    if multilabel:
        y_pred[y_pred > 0] = 1
        y_pred[y_pred <= 0] = 0
    else:
